...
ret
```
After compiling and testing the shellcode, it still worked. Although there was one flaw. It added a bunch of `0x00` bytes to our shellcode.  
...
```
root# objdump -D offASLR.poly6 | grep '[0-9a-f]:' | grep -v 'file' | cut -f2 -d: | cut -f1-6 -d' ' | tr -s ' ' | tr '\t' ' ' | sed 's/ $//g' | sed 's/ /\\x/g' | paste -d '' -s | sed 's/^/"/' | sed 's/$/"/g'
 ""
```
